    ...
    <aside v-show="$page.props.showingMobileMenu" class="fixed inset-y-0 z-20 flex-shrink-0 w-64 mt-16 overflow-y-auto bg-white md:hidden">
      <div class="py-4 text-gray-500">
        ...
        <ul class="mt-6">
          ...
          <li class="relative px-6 py-3">
            <ResponsiveNavLink :href="route('platforms.index')" :active="route().current('platforms.index')">
              <template #icon>
                <svg class="w-5 h-5" aria-hidden="true" fill="none" stroke-linecap="round"
                     stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" stroke="currentColor">
                  <path
        ...
        <li class="relative px-6 py-3">
          <button @click="showingTwoLevelMenu = !showingTwoLevelMenu"
              ...
              aria-haspopup="true">
                <span class="inline-flex items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                    ...
                    <span class="ml-4">Videogames</span>
                </span>
            ...
            <ul v-show="showingTwoLevelMenu || route().current('videogames.index') ||route().current('videogames.create')" class="p-2 mt-2 space-y-2 overflow-hidden text-sm font-medium text-gray-500 rounded-md shadow-inner bg-gray-50"
                aria-label="submenu">
              <li class="px-2 py-1 transition-colors duration-150 hover:text-gray-800">
                <ResponsiveNavLink :href="route('videogames.index')" :active="route().current('videogames.index')" >
                  List
                </ResponsiveNavLink>
              </li>
              <li class="px-2 py-1 transition-colors duration-150 hover:text-gray-800">
                <ResponsiveNavLink :href="route('videogames.create')" :active="route().current('videogames.create')" >
                  Create
                </ResponsiveNavLink>
              </li>
            </ul>
